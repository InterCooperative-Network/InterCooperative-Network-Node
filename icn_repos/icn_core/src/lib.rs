...
impl IcnNode {
    ...
    pub async fn start(&self) -> IcnResult<()> {
        ...
        self.sharding_manager.write().await.start()?;
        Ok(())
    }
    ...
    pub async fn stop(&self) -> IcnResult<()> {
        ...
        self.sharding_manager.write().await.stop()?;
        Ok(())
    }
    ...
    pub async fn process_transaction(&self, transaction: Transaction) -> IcnResult<()> {
        ...
        self.sharding_manager.write().await.process_transaction(shard_id, &transaction).await?;
        Ok(())
    }
    ...
    pub async fn create_proposal(&self, proposal: Proposal) -> IcnResult<String> {
        ...
        self.network_manager.read().await.broadcast_proposal(&proposal_id).await?;
        Ok(proposal_id)
    }
    ...
    pub async fn get_network_stats(&self) -> IcnResult<NetworkStats> {
        self.network_manager.read().await.get_stats().await
    }
...
mod tests {
   ...
   async fn test_sharding() {
       ...
       let address1 = "Alice".to_string();
       let address2 = "Bob".to_string();

       let shard1 = node.get_shard_for_address(&address1).await;
       let shard2 = node.get_shard_for_address(&address2).await;

       assert!(shard1 < node.config.shard_count);
       assert!(shard2 < node.config.shard_count);

       // Test cross-shard transaction
       node.mint_currency(&address1, &CurrencyType::BasicNeeds, 1000.0).await.unwrap();
       ...
       // Check balances across shards
       let alice_balance = node.get_total_balance(&address1, &CurrencyType::BasicNeeds).await.unwrap();
       let bob_balance = node.get_total_balance(&address2, &CurrencyType::BasicNeeds).await.unwrap();
